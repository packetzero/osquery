...
def run_http_server(bind_port=80, **kwargs):
    ...
    if ARGS['ipv6']:
        httpd = HTTPServerV6(('::0', bind_port), RealSimpleHandler)
    else:
        httpd = HTTPServer(('0.0.0.0', bind_port), RealSimpleHandler)
    if ARGS['tls']:
        if 'SSLContext' in vars(ssl):
            ...
            httpd.socket = ssl.wrap_socket(
                ...
                keyfile=ARGS['key'],
                server_side=True)
    debug("Starting %s server on TCP %s port: %d" % ( ("TLS/HTTPS" if ARGS['tls'] else "HTTP"), ("IPV6" if ARGS['ipv6'] else ""), bind_port))
